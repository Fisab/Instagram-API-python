...
class InstagramAPI:
	...
	def __init__(self, username, password, logg, debug = False, IGDataPath = None):
		m = hashlib.md5()
	...
	def save_session(self, user):
		user['username_id'] = self.username_id
		user['rank_token'] = self.rank_token
		user['token'] = self.token
		user['uuid'] = self.uuid

		return user

	def load_session(self, user, cookies, log_id):
		if 'rank_token' in user:
			self.s = requests.Session()
			self.s.cookies = cookies

			self.isLoggedIn = True
			self.username_id = user['username_id']
			self.rank_token = user['rank_token']
			...
			if 'status' in self.LastJson and self.LastJson['status'] == 'fail':
				self.loger.log('#', log_id, 'Error with user', user['login'])
				return False
			self.loger.log('#', log_id, 'Succes load session', user['login'])
	...
	def set_proxy(self, ip):
		"""
		Set ip: {"https":"http:[<ipv6:address>]:<portnumber>"}
		"""
		try:
			self.s
		except:
	...
	def SendRequest(self, endpoint, post = None, login = False):
		#fisab
		if int(time.time()) - self.last_req < 3:
			time.sleep(5)
		self.last_req = int(time.time())
		#
		...
		if response.status_code == 200:
			...
			print ("Request return " + str(response.status_code) + " error!", endpoint)
			# for debugging
			try:
				self.LastResponse = response
				self.LastJson = json.loads(response.text)
				print(self.LastJson, endpoint, self.username)
			except:
				pass
			return False
